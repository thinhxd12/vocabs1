<script lang="ts">
  import { locationList } from "$lib/store/navstore";
...
<div
  class="w-content min-h-[284px] layout-light rounded-3 !overflow-hidden absolute bottom-0 right-0 py-6"
  transition:fly={{ y: "100%", opacity: 0.5, easing: quintOut }}
>
  <select
    bind:value={location}
